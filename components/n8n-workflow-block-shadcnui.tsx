...
export function N8nWorkflowBlock() {
  ...
  const contentWidth = Math.max(...nodes.map((n) => n.position.x + NODE_WIDTH)) + 50;
  const contentHeight = Math.max(...nodes.map((n) => n.position.y + NODE_HEIGHT)) + 50;

  return (
    <div className="relative w-full overflow-hidden rounded-2xl p-4 sm:p-6">
      {/* Canvas */}
      <div
        className="relative h-[400px] w-full overflow-auto rounded-xl bg-background/40 sm:h-[500px] md:h-[600px]"
